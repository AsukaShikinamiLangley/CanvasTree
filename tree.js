...
class Tree {
  ...
  createSideTree() {
    ...
    const minHeight =
      Math.max(...[this.leftTree[depth].length, this.rightTree[depth].length]) * (fontSize + 4);
  ...
  render(role = 'platform') {
    ...
    const centerNode = {
      name: 'root',
      position: {
        x: this.columnPositionArr[Math.floor(this.columnPositionArr.length / 2)],
        y: this.height / 2,
      },
      layer: 0,
    ...
    const renderCenterNode = node => {
      ...
      const leftMaxSubWidth = Math.max(
        ...node[childKey].filter(sub => sub.partOf === 'left').map(sub => sub.width)
      );
      const rightMaxSubWidth = Math.max(
        ...node[childKey].filter(sub => sub.partOf === 'right').map(sub => sub.width)
      );
